...
Default recording time when recording to WAV if Songlengths.md5 isn't found. Defaults to 4 minutes.

=item B<Kernal ROM>=I<< <path> >>

Full path for the Kernal ROM file. Out of all the 3 ROM files, this is the most important one, and thus should be provided as some tunes may not play without it.
...
2 for a more detailed one (register dumps, displaying the notes being played and waveforms + control registers being used);
...
Enable DigiBoost, a hack for the 8580 SID chip to make digi samples originally meant for the 6581 audible.
